...
public class BlinkManager {
    ...
    /**
     * Mirrors Rise's {@code BlinkComponent.bf()}: flush every packet held by the
     * current blink session and immediately reopen the blink channel for the
     * same module, so the next round of block/unblock packets is held again.
     * <p>
     * Unlike {@link #setBlinkState(boolean, BlinkModules)} with {@code false},
     * this leaves {@code blinking == true} and {@code blinkModule} unchanged,
     * which is required by the Watchdog AutoBlock cycle (Rise flushes the old
     * ticket and opens a new one in the same call).
     */
    public boolean flushAndRestartBlink(BlinkModules module) {
        if (module == BlinkModules.NONE) {
            return false;
        }
        if (this.blinkModule != module && this.blinkModule != BlinkModules.NONE) {
            return false;
        }
        this.blinking = false;
        if (Minecraft.getMinecraft().getNetHandler() != null && !this.blinkedPackets.isEmpty()) {
