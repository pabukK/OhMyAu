        ...
        if (this.isEnabled() && this.canBoost()) {
            if (mc.thePlayer.onGround) {
                ...
                if (this.strafe.getValue() > 0) {
                    ...
                    // Rise WatchdogPredictionSpeed: while KillAura is blocking, air-strafe is
                    // reduced (moveFlying 0.063 vs 0.087) so the client doesn't out-move the block.
                    double blockFactor = KillAura.blocking ? 0.72 : 1.0;
                    MoveUtil.setSpeed(speed * (double) ((float) (100 - this.strafe.getValue()) / 100.0F) * blockFactor, MoveUtil.getDirectionYaw());
                    MoveUtil.addSpeed(
                            speed * (double) ((float) this.strafe.getValue().intValue() / 100.0F) * blockFactor, MoveUtil.getMoveYaw()
                    );
