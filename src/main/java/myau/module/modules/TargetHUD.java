...
public class TargetHUD extends Module {
    ...
    /**
     * Renders the TargetHUD with its top-left corner anchored at the given
     * scaled resolution position.
     *
     * @param x       unscaled screen x of the top-left corner
     * @param y       unscaled screen y of the top-left corner
     * @param preview whether to render a preview used by the HUD editor while
     *                the mouse cursor is visible (e.g. the chat is open). The
     *                preview simply renders the original TargetHUD with the
     *                player themselves as the target, so it looks exactly like
     *                the real thing and can be dragged to the desired spot even
     *                when no enemy is locked.
     * @return the rendered bar width (unscaled), or 0 if nothing was drawn
     */
    public float renderAt(float x, float y, float partialTicks, boolean preview) {
        if (mc.thePlayer == null) {
            return 0.0F;
        }
        if (!preview && !this.isEnabled()) {
            return 0.0F;
        }
