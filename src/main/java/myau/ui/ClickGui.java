...
public class ClickGui extends GuiScreen {
    ...
    public void mouseClicked(int x, int y, int mouseButton) {
        // Only the topmost category under the cursor handles the click, otherwise
        // overlapping categories would all react to the same click and toggle
        // unrelated modules.
        CategoryComponent category = null;
        for (int i = categoryList.size() - 1; i >= 0; i--) {
            CategoryComponent c = categoryList.get(i);
    ...
    public void keyTyped(char typedChar, int key) {
        if (key == 1 && !BindComponent.isAnyBinding()) {
            this.mc.displayGuiScreen(null);
            return;
        }

        if (key == 1) {
            // ESC pressed while a key bind is listening: forward it to every
            // component (including closed categories) so the bind is cleared,
            // and keep the GUI open.
            for (CategoryComponent category : categoryList) {
    ...
    public void onGuiClosed() {
        // Never leave a half-finished key bind listening after the GUI closes
        BindComponent.stopAllBinding();
        savePositions();
    }
