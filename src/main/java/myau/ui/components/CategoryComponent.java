...
public class CategoryComponent {
    ...
    public void render(FontRenderer renderer) {
        ...
        // Reset visibility: only modules actually drawn inside the scroll window
        // may receive clicks this frame.
        for (Component c : this.modulesInCategory) {
            ((ModuleComponent) c).inView = false;
        }
        ...
        if (this.categoryOpened && !this.modulesInCategory.isEmpty()) {
            ...
            for (Component c2 : this.modulesInCategory) {
                ...
                if (renderHeight + compHeight > animScroll &&
                    ...
                    ((ModuleComponent) c2).inView = true;
    ...
    /**
     * Whether the point lies inside this category's drawn area (header, or the
     * visible content window when the category is open). Used to pick the
     * topmost category for a click so overlapping categories don't all react.
     */
    public boolean containsPoint(int x, int y) {
        if (x >= this.x - 2 && x <= this.x + this.width + 2 && y >= this.y && y <= this.y + this.bh + 3) {
            return true;
        }
        if (this.categoryOpened && !this.modulesInCategory.isEmpty()) {
            int displayHeight = Math.min(height, MAX_HEIGHT);
            if (x >= this.x - 1 && x <= this.x + this.width + 1 &&
                    y >= this.y + this.bh + 3 && y <= this.y + this.bh + 3 + displayHeight) {
                return true;
            }
        }
        return false;
